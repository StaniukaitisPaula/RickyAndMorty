...
fun Greeting(name: String) {
    ...
    Column(modifier = Modifier.padding(32.dp)) {
        Button(onClick = {
            ...
            // chamada para a API
            val call = RetrofitFactory().getCharacterService().getCharactersByPage(1)
        ...
        LazyColumn(){
            items(results){
                Card(modifier = Modifier
                    .fillMaxWidth()
                    .padding(top = 8.dp),
                    backgroundColor = Color(0x4FF91C253)
                    ) {
                    Row( modifier = Modifier.padding(8.dp)) {
                       Card(shape = CircleShape) {
                           AsyncImage(model = it.image, contentDescription = "avatar",
